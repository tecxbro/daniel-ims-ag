...
const MEMORY_ID_SALT_PATTERN = /^[a-f0-9]{64}$/;
...
export function isValidMemoryIdSalt(value: unknown): value is string {
  return typeof value === "string" && MEMORY_ID_SALT_PATTERN.test(value.trim());
}
...
function requireSalt(explicitSalt?: string): string {
  ...
  const normalized = salt.trim();
  if (!isValidMemoryIdSalt(normalized)) {
    throw new MemoryIdentityConfigurationError(
      "DANIEL_MEMORY_ID_SALT must be the 32-byte lowercase hexadecimal value generated by setup",
    );
  }
  return normalized;
}
...
/** Server-only proof used to authorize primary-owner reads and registration. */
export function memoryPairingAuthorityProof(explicitSalt?: string): string {
  const salt = requireSalt(explicitSalt);
