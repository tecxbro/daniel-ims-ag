...
| `scripts/setup.ts` | Interactive setup for Photon project ID and project secret |
...
- Inbound replies and progress acknowledgements reuse the inbound Space.
- Proactive direct messages resolve with `im.space(user)` and do not select a
  configured phone-line override.
- `message.sender.id` remains the inbound user identity.
- Normalized SMS conversations are preserved for automations, drafts, and
  explicit local primary-owner pairing. The paired conversation is the only
  destination for proactive Gmail notices.
