...
Run `/upgrade-daniel` inside the repo from `claude`. This is the supported upgrade path: the agent previews the diff, creates rollback points, performs the merge, resolves conflicts when needed, and runs referenced migration skills.
