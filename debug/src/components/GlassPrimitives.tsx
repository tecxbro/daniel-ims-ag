...
export function SegmentedControl<T extends string>({
  ...
  function moveToEndpoint(endpoint: "first" | "last") {
    const indices = Array.from({ length: options.length }, (_, index) => index);
    if (endpoint === "last") indices.reverse();
    const next = indices.find((index) => !isDisabled(index));
    if (next === undefined) return;
    buttonRefs.current[next]?.focus();
    if (options[next].value !== value) void onChange(options[next].value);
  }
    ...
    >
      {options.map((option, index) => (
        <button
          ...
          aria-controls={role === "tablist" ? option.controls : undefined}
