...
describe("memorySyncJobs Convex transactions", () => {
  it("deduplicates a turn and rejects conflicting payloads", async () => {
    ...
    const first = await t.mutation(api.memorySyncJobs.enqueue, enqueueArgs());
    const duplicate = await t.mutation(
      api.memorySyncJobs.enqueue,
  ...
  it("canonicalizes uppercase SHA-256 hashes before deduplication", async () => {
    ...
    const duplicate = await t.mutation(api.memorySyncJobs.enqueue, enqueueArgs({
      jobId: "job_duplicate",
    }));
  ...
  it("transactionally gives a due attempt to only one worker", async () => {
    const t = convexTest(schema, modules);
    await t.mutation(api.memorySyncJobs.enqueue, enqueueArgs());

    const [first, second] = await Promise.all([
      t.mutation(api.memorySyncJobs.claimDue, {
        now: 1_000,
        leaseMs: 120_000,
        workerId: "worker_a",
      }),
  ...
  it("persists provider IDs and completes submitted work after restart", async () => {
    ...
    await t.mutation(api.memorySyncJobs.recordSubmitted, {
      jobId: "job_001",
      expectedAttempt: 1,
    ...
    const resumed = await t.mutation(api.memorySyncJobs.claimDue, {
      now: 121_001,
      leaseMs: 120_000,
    ...
    await t.mutation(api.memorySyncJobs.complete, {
      jobId: "job_001",
      expectedAttempt: resumed!.job.attempts,
      expectedUpdatedAt: resumed!.job.updatedAt,
      now: 121_002,
    });
    const [job] = await t.query(api.memorySyncJobs.list, {
      status: "completed",
      limit: 10,
    });
  ...
  it("enforces the fixed retry schedule and dead-letter transition", async () => {
    ...
    for (let attempt = 1; attempt <= 5; attempt += 1) {
      ...
      const result = await t.mutation(api.memorySyncJobs.recordFailure, {
        jobId: "job_001",
    ...
    const [job] = await t.query(api.memorySyncJobs.list, { status: "dead_letter", limit: 10 });
  ...
  it("reclaims an expired lease and rejects stale completion", async () => {
    const t = convexTest(schema, modules);
    await t.mutation(api.memorySyncJobs.enqueue, enqueueArgs());
    const first = await t.mutation(api.memorySyncJobs.claimDue, {
      now: 1_000,
      leaseMs: 10_000,
      workerId: "worker_a",
    });
    expect(await t.mutation(api.memorySyncJobs.claimDue, {
      now: 10_999,
      leaseMs: 10_000,
      workerId: "worker_b",
    })).toBeNull();
    const reclaimed = await t.mutation(api.memorySyncJobs.claimDue, {
      now: 11_000,
      leaseMs: 10_000,
      workerId: "worker_b",
    });
    ...
    expect(await t.mutation(api.memorySyncJobs.complete, {
      jobId: "job_001",
  ...
  it("initializes the deployment salt fingerprint once", async () => {
    const t = convexTest(schema, modules);
    expect(await t.mutation(api.memoryProviderState.ensureIdentitySaltFingerprint, {
      saltFingerprint: "fingerprint_a",
      now: 1,
    })).toBe("fingerprint_a");
    expect(await t.mutation(api.memoryProviderState.ensureIdentitySaltFingerprint, {
      saltFingerprint: "fingerprint_b",
      now: 2,
    })).toBe("fingerprint_a");
  });
});
